...
// @desc    Get all available recycle requests (for recyclers)
...
exports.getAvailableRecycles = async (req, res, next) => {
  try {
    ...
    const recycles = await Recycle.find({ status: 'AVAILABLE' })
      .populate('userId', 'name phone email')
      .select('-assignedRecycler')
      .sort({ createdAt: -1 })
      .limit(parseInt(limit))
      .skip(skip)
      .lean();

    const total = await Recycle.countDocuments({ status: 'AVAILABLE' });

    console.log(`✅ Fetched ${recycles.length} available recycle requests`);
    ...
    console.error('❌ Error fetching available recycles:', error);
    next(error);
  }
};

// @desc    Accept recycle request (by recycler)
// @route   POST /api/recycle/:id/accept
// @access  Public (called by recycler backend)
exports.acceptRecycle = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { acceptedBy } = req.body;

    console.log(`📥 Accept request received - ID: ${id}, Recycler: ${acceptedBy}`);

    if (!acceptedBy) {
      return next(new AppError('Recycler ID is required', 400));
    }

    const recycleRequest = await Recycle.findById(id);
    console.log(`🔍 Looking for recycle with ID: ${id}`);
    console.log(`📊 Found recycle:`, recycleRequest ? 'YES' : 'NO');

    if (!recycleRequest) {
      console.log(`❌ Recycle not found with ID: ${id}`);
      return next(new AppError('Recycle request not found', 404));
    }

    if (recycleRequest.status !== 'AVAILABLE') {
      return next(new AppError('This request has already been accepted', 400));
    }

    // Update the recycle request
    recycleRequest.status = 'ASSIGNED';
    recycleRequest.assignedRecycler = acceptedBy;
    await recycleRequest.save();

    console.log(`✅ Recycle request ${id} accepted by recycler ${acceptedBy}`);
