...
import { Users, Recycle, Star, TrendingUp, AlertCircle, CheckCircle, Phone, Mail } from 'lucide-react';
import axios from 'axios';

const NGORecyclerMonitoring = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [ngosRes, recyclersRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_BASE_URL}/ngos?limit=50`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        axios.get(`${import.meta.env.VITE_API_BASE_URL}/recyclers?limit=50`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Track NGO and Recycler performance</p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="card bg-gradient-to-br from-yellow-50 to-white">
              ...
              <p className="text-3xl font-bold text-yellow-600">
                {(ngos.reduce((sum, n) => sum + n.rating, 0) / ngos.length).toFixed(1)}
              </p>
            </div>
                      ...
                      <tr key={ngo._id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">{ngo.address.city}, {ngo.address.state}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col items-center">
                            <div className="flex gap-1 mb-1">
                              {getRatingStars(Math.round(ngo.rating))}
                            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="card bg-gradient-to-br from-yellow-50 to-white">
              <p className="text-sm text-gray-600 mb-1">Avg Rating</p>
              <p className="text-3xl font-bold text-yellow-600">
                {(recyclers.reduce((sum, r) => sum + r.rating, 0) / recyclers.length).toFixed(1)}
              </p>
            </div>
            <div className="card bg-gradient-to-br from-purple-50 to-white">
              <p className="text-sm text-gray-600 mb-1">Active</p>
                      ...
                      <tr key={recycler._id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">{recycler.address.city}, {recycler.address.state}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold capitalize">
                            {recycler.facilityType.replace('_', ' ')}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2 text-sm">
                              <Mail size={14} className="text-gray-400" />
                              <span className="text-gray-700">{recycler.email}</span>
                            </div>
                            <div className="flex items-center gap-2 text-sm">
                              <Phone size={14} className="text-gray-400" />
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col items-center">
                            <div className="flex gap-1 mb-1">
                              {getRatingStars(Math.round(recycler.rating))}
                            </div>
                            <span className="text-sm font-semibold text-gray-700">
                              {recycler.rating.toFixed(1)} / 5.0
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <p className="text-sm font-semibold text-eco-main">
                            {recycler.performanceMetrics.totalMaterialProcessed.toFixed(2)} kg
                          </p>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center">
...
export default NGORecyclerMonitoring;