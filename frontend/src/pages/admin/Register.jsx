...
const Register = () => {
  ...
  const validateForm = () => {
    ...
    // Email validation - ONLY @ecoloop.com allowed
    // No other email domains are permitted
    if (!formData.email) {
      newErrors.email = 'Email is required';
    } else if (!/^[^\s@]+@ecoloop\.com$/.test(formData.email)) {
      newErrors.email = 'Only @ecoloop.com email addresses are allowed. Example: admin@ecoloop.com';
    }
  ...
  const googleRegister = useGoogleLogin({
    onSuccess: async (codeResponse) => {
      try {
        ...
        // Send token to backend for verification and auto-registration
        ...
        const errorMsg = error.response?.data?.message || error.message || 'Google registration failed';
    ...
    <div className="min-h-screen bg-gradient-to-br from-eco-light to-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="card shadow-lg">
          <h2 className="text-2xl font-bold text-eco-dark mb-6 text-center">Create Admin Account</h2>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <p className="text-center text-sm text-gray-600">
            ...
            <a href="/admin/login" className="text-eco-main hover:text-eco-dark font-semibold">
              Login here
            </a>
          </p>
        </div>
