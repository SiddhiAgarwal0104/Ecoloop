...
const Dashboard = () => {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      // ✅ ROLE BASED ENDPOINT (ONLY REAL CHANGE)
      let endpoint = '/dashboard/household';
      if (user?.role === 'RECYCLER') endpoint = '/recycler/dashboard';
      if (user?.role === 'NGO') endpoint = '/dashboard/ngo';
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link
                  to="/donations"
                  className="text-eco-main font-semibold hover:text-eco-dark"
                >
                  View All
                </Link>
              </div>
                ...
                {dashboardData?.recycles?.slice(0, 5).map((recycle) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-gray-800">
                          {recycle.wasteCategory}
                        </p>
                        <p className="text-sm text-gray-500">
                          {recycle.quantity} {recycle.unit}
                        </p>
                      </div>
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          recycle.status === 'AVAILABLE'
                            ? 'bg-green-100 text-green-700'
                            : recycle.status === 'ACCEPTED'
                            ? 'bg-yellow-100 text-yellow-700'
                            : recycle.status === 'PICKED_UP'
                            ? 'bg-blue-100 text-blue-700'
                            : 'bg-gray-100 text-gray-700'
                        }`}
                      >
