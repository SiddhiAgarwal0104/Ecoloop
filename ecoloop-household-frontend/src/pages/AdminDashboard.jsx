...
const AdminDashboard = () => {
  ...
  const fetchPlatformStats = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/stats/platform`, {
  ...
  const fetchAdminProfile = async () => {
    try {
      ...
      setAdminProfile(response.data.data?.user || response.data.user || response.data.data);
    } catch (err) {
  ...
  return (
    <AdminLayout>
      ...
      {/* Admin Profile Card */}
      {adminProfile && (
        <div className="mb-8 bg-white rounded-xl shadow p-5 border-l-4 border-eco-main flex items-center gap-5">
          <div className="w-14 h-14 rounded-full bg-eco-main flex items-center justify-center text-white text-2xl font-bold flex-shrink-0">
            {adminProfile.name?.charAt(0)?.toUpperCase() || 'A'}
          </div>
          <div>
            <p className="text-xs text-gray-500 font-semibold uppercase tracking-wide">Logged in as Admin</p>
            <p className="text-xl font-bold text-gray-900">{adminProfile.name}</p>
            <p className="text-sm text-gray-600">{adminProfile.email}</p>
            <div className="flex gap-4 mt-1">
              {adminProfile.city && (
                <span className="text-xs bg-eco-light text-eco-dark px-2 py-1 rounded-full font-semibold">
                  🏙️ {adminProfile.city}
                </span>
              )}
              {adminProfile.locality && (
