...
// CNN FastAPI URL — same machine as backend
const CNN_API_URL = import.meta.env.VITE_CNN_API_URL || 'http://localhost:8000';

/**
 * Map CNN detected class → tips (mirrors backend CNN_CLASS_MAP)
 */
const CNN_TIPS = {
  battery:    ['🔋 Take to e-waste collection center', '☠️ Never throw in regular trash', '♻️ Many stores accept used batteries', '⚡ Contains hazardous materials'],
  biological: ['🌱 Can be composted', '🌍 Reduces landfill waste', '🌿 Creates nutrient-rich soil', '♻️ Best for environment'],
  cardboard:  ['📦 Flatten all boxes before recycling', '🚿 Keep dry and clean', '🏷️ Remove tape and staples', '✅ Highly recyclable'],
  clothes:    ['👕 Donate if still wearable', '♻️ Textile recycling bins available', '✂️ Old clothes can be repurposed', '🌍 Reduces fashion waste'],
  glass:      ['🔴 100% recyclable', '🎨 Separate by color if possible', '🏷️ Remove labels', '✅ Excellent recycling material'],
  metal:      ['💪 Crush cans to save space', '🧲 Highly magnetic material', '✨ Very valuable for recycling', '💰 Best recycling value'],
  paper:      ['📄 Keep dry and clean', '✂️ Flatten to save space', '🏷️ Remove plastic windows from envelopes', '🔐 Good for recycling'],
  plastic:    ['♻️ Rinse plastic before recycling', '🏷️ Remove labels and caps', '📦 Flatten to save space', '✅ Check recycling number on bottom'],
  shoes:      ['👟 Donate if still wearable', '🏪 Some brands have take-back programs', '♻️ Specialized shoe recyclers exist', '🌍 Keep out of landfill'],
  trash:      ['⚠️ General waste — sort before discarding', '🔍 Check if any items can be separated', '🗑️ Use general waste bin', '🌍 Minimize mixed waste generation'],
};

const CLASS_TO_CATEGORY = {
  battery: 'E_WASTE', biological: 'ORGANIC', cardboard: 'PAPER',
  clothes: 'MIXED', glass: 'GLASS', metal: 'METAL',
  paper: 'PAPER', plastic: 'PLASTIC', shoes: 'MIXED', trash: 'MIXED',
};
...
const CreateRecycle = () => {
  ...
  /**
   * Call CNN FastAPI directly from browser with image file
   */
  const runCNNDetection = async (file, imageId, isFirst) => {
    setDetectingImage(imageId);
    try {
      ...
      // result = { category: "plastic", confidence: 0.93 }

      const classKey = result.category?.toLowerCase() || 'trash';
      const tips = CNN_TIPS[classKey] || CNN_TIPS.trash;
      const recyclable = !['shoes', 'trash'].includes(classKey);
      ...
      setImageDetections(prev => ({ ...prev, [imageId]: detection }));

      // Auto-fill waste category from first image
      if (isFirst) {
        const category = CLASS_TO_CATEGORY[classKey] || 'MIXED';
        setFormData(prev => ({ ...prev, wasteCategory: category }));
      }
    } catch (err) {
      console.error('⚠️ CNN detection failed:', err.message);
      setImageDetections(prev => ({
        ...prev,
        [imageId]: { detected: false, error: 'Detection failed' }
      }));
    } finally {
      setDetectingImage(null);
    }
  ...
  const handleImageChange = (e) => {
    ...
    files.forEach((file, index) => {
      ...
      // Run real CNN detection
      const isFirst = images.length === 0 && index === 0;
      runCNNDetection(file, imageId, isFirst);
    });
  ...
  return (
    <div className="fade-in max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="card mb-6 border-2 border-green-400 bg-gradient-to-r from-green-50 to-emerald-50">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className={`rounded-lg p-3 mb-4 border-2 ${aiDetection.recyclable ? 'bg-green-50 border-green-300' : 'bg-orange-50 border-orange-300'}`}>
                <p className="text-sm font-semibold mb-1">
                  {aiDetection.recyclable ? '♻️ Recyclable Material' : '⚠️ Non-Recyclable'}
                </p>
                <p className={`text-sm ${aiDetection.recyclable ? 'text-green-700' : 'text-orange-700'}`}>
        ...
        <div className="card mb-6 border-2 border-yellow-400 bg-yellow-50">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-yellow-800 mb-1">⚠️ AI Detection Could Not Process Image</h3>
              <p className="text-sm text-yellow-700">
                The AI couldn't identify the waste type. Please check image quality or select category manually.
              </p>
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Waste Category *</label>
            <select name="wasteCategory" value={formData.wasteCategory} onChange={handleChange} className="input-field" required>
              <option value="PLASTIC">Plastic</option>
              <option value="PAPER">Paper</option>
              <option value="METAL">Metal</option>
              <option value="GLASS">Glass</option>
          ...
          <div>
            ...
            <select name="wasteType" value={formData.wasteType} onChange={handleChange} className="input-field" required>
              <option value="SEGREGATED">Segregated</option>
              <option value="MIXED">Mixed</option>
            </select>
          </div>
          <div>
            ...
            <input type="number" name="quantity" value={formData.quantity} onChange={handleChange} className="input-field" min="0.1" step="0.1" required />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea name="description" value={formData.description} onChange={handleChange} className="input-field" rows={4} placeholder="Add details about the waste..." />
        </div>

        {/* Image Upload */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-semibold text-gray-700">Upload Images (Max 5)</label>
            <span className="text-xs bg-eco-main text-white px-2 py-1 rounded-full font-semibold">🤖 CNN AI Detection</span>
          </div>
          <p className="text-xs text-gray-600 mb-3">
            Upload clear photos of your waste. Our CNN model will automatically detect the type.
          </p>
          <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-eco-main transition-colors">
            <Upload className="mx-auto text-gray-400 mb-2" size={40} />
            <input type="file" accept="image/*" multiple onChange={handleImageChange} className="hidden" id="image-upload" />
            <label htmlFor="image-upload" className="text-eco-main font-semibold cursor-pointer hover:text-eco-dark">Click to upload</label>
            <p className="text-sm text-gray-500 mt-1">PNG, JPG, WEBP up to 5MB each</p>
            {images.length > 0 && (
              <p className="text-xs text-eco-main font-semibold mt-2">
                📷 {images.length} image{images.length !== 1 ? 's' : ''} added
                  ...
                  return (
                    <div key={imageId} className="relative group">
                      <div className="w-full h-32 bg-gray-100 rounded-lg overflow-hidden relative">
                        ...
                        {detection && detection.detected && !isDetecting && (
                          <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-2 rounded-lg">
                            <Sparkles className="w-5 h-5 text-yellow-300 mb-1" />
                            <span className="text-white text-xs font-bold text-center mb-1">🤖 AI Detected</span>
                            <span className="text-yellow-300 text-xs font-semibold">{detection.wasteType}</span>
                            <span className="text-green-300 text-xs">{Math.round(detection.confidence * 100)}% confident</span>
                          </div>
                        )}
                        {detection && !detection.detected && !isDetecting && (
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded-lg">
                            <span className="text-yellow-300 text-xs font-semibold text-center px-2">⚠️ Detection failed</span>
                          </div>
                        )}
                      </div>
                      <button
        ...
        <div className="flex gap-4">
          <button type="button" onClick={() => navigate('/recycles')} className="btn-secondary flex-1">Cancel</button>
