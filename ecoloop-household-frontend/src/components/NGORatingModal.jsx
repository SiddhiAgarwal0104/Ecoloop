...
const NGORatingModal = ({ donation, ngoInfo, ratingType, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      console.log('⭐ [NGO Rating Modal] Submitting rating with:', {
      ...
      const response = await ngoRatingsAPI.submitRating(
        donation._id,
        ngoInfo._id,
        rating,
        feedback,
      ...
      console.error('⭐ [NGO Rating Modal] Error:', err.response?.data || err);
  ...
  const ratingTitle = ratingType === 'ITEM_RECEIVED' 
    ? 'Rate your experience with this NGO' 
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-96 overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-eco-main to-eco-dark text-white p-6 flex justify-between items-start">
          <div>
            ...
            <p className="text-eco-light text-sm mt-1">{ngoInfo.name}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            <p className="text-sm text-gray-600">
              <span className="font-semibold">Item:</span> {donation.itemCategory}
            </p>
            <p className="text-sm text-gray-600">
